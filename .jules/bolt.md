...
## 2026-04-06 - Saturating the iOS Bluetooth Write Buffer for Peak Throughput
**Learning:** On iOS, 'peripheralIsReady(toSendWriteWithoutResponse:)' is triggered when the internal buffer begins to clear. Sending only a single message per callback results in suboptimal throughput due to delegate overhead. Using a 'while' loop to drain the pending message queue until 'canSendWriteWithoutResponse' returns 'false' ensures the buffer remains saturated, maximizing effective throughput. In template-only repositories where benchmarks are impossible, theoretical performance analysis (e.g., reducing O(n) context switches) is critical for documenting "Developer Time to Action" (DX) improvements.
**Action:** Always use a 'while' loop within 'peripheralIsReady' to saturate the write-without-response buffer and minimize context-switching between the app and the Bluetooth stack. Document expected impact (e.g., ~2-5x reduction in callback overhead) to guide implementation.
